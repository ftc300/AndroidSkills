package com.ftc300.androidskills.async;
...
import com.inso.async.*;
import com.inso.async.SafeTask;
import com.inso.async.SimpleCachedTask;
import com.inso.async.SimpleSafeTask;
...
//				}
	...
	private void testOrderedTaskExecutor() {
		...
		SimpleTask<?> lastTask = new SimpleTask<Integer>() {
			@Override
			protected Integer doInBackground() {
				return 12;
			}
			...
			@Override
			protected void onPostExecute(Integer result) {
				Log.i("result:"+ result + ",OrderedTaskExecutor use time: " + (System.currentTimeMillis() - startTime));
